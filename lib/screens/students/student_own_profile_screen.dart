...
class _StudentOwnProfileScreenState extends State<StudentOwnProfileScreen> {
  ...
  Future<void> _loadStudentData() async {
    ...
    log.d("ACCESS_TOKEN: \\${await ApiService.getToken()}");
    ...
    try {
      ...
      if (data != null) {
        ...
        log.t("[StudentOwnProfile] response: \\${data.toString()}");  // Using trace instead of verbose
      } else {
        log.i(
      ...
      if (!mounted) return;
      log.e(
        "StudentOwnProfileScreen: Error cargando datos",
        error: e,
        stackTrace: s,
      );
      setState(() {
  ...
  String _formatDateString(String? rawIsoDateString) {
    ...
    try {
      ...
      log.w("Fecha inválida $rawIsoDateString: $e");
  ...
  Future<void> _uploadSignedConsent() async {
    ...
    try {
      ...
      if (!mounted) return;
      log.e(
        "StudentOwnProfileScreen: Excepción al subir consentimiento",
        error: e,
        stackTrace: s,
      );
      ScaffoldMessenger.of(context).showSnackBar(
