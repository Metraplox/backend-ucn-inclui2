...
export class HawaiiSyncController {
  ...
  async syncNeeStudents(@Body() body: { semester: string }) {
    this.logger.log(
      `Iniciando sincronización de estudiantes NEE para semestre ${body.semester}`,
    );
  ...
  @Roles(UserRole.COORDINADOR)
  @ApiOperation({
    summary: 'Sincronizar cursos e inscripciones de estudiantes NEE',
  })
  @ApiResponse({
    status: 200,
  ...
  async syncCoursesAndEnrollments(@Body() body: { semester: string }) {
    this.logger.log(
      `Iniciando sincronización de cursos e inscripciones para semestre ${body.semester}`,
    );
  ...
  async syncAllNeeData(@Body() body: { semester: string }) {
    this.logger.log(
      `Iniciando sincronización completa de datos NEE para semestre ${body.semester}`,
    );
