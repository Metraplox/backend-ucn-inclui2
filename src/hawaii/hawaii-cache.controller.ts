...
export class HawaiiCacheController {
  ...
  @ApiOperation({ 
    ...
    description: 'Muestra información detallada sobre el estado y uso del caché'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      properties: {
        memoryCacheSize: { type: 'number', description: 'Número de elementos en caché memoria' },
        diskCacheFiles: { type: 'number', description: 'Archivos de caché en disco' },
        totalDiskSize: { type: 'number', description: 'Tamaño total en bytes' },
        oldestFile: { type: 'string', nullable: true, description: 'Archivo más antiguo' },
        newestFile: { type: 'string', nullable: true, description: 'Archivo más reciente' },
  ...
  @ApiOperation({ 
    ...
    description: 'Descarga y cachea todos los datos (estudiantes, cursos, inscripciones) para un semestre específico'
  })
  @ApiParam({ name: 'semester', description: 'Semestre académico (formato: YYYYPP)', example: '20251' })
  @ApiResponse({ 
    status: 200, 
  ...
  async forceRefresh(
    ...
    this.logger.log(`🔄 Forzando actualización de caché: ${body.type}${body.semester ? ` para semestre ${body.semester}` : ''}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Elimina archivos de caché antiguos y optimiza el almacenamiento'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Muestra si un tipo de dato específico está cacheado y cuándo se actualizó'
  })
  @ApiParam({ 
    name: 'type', 
  ...
  async getCacheStatus(
    ...
    this.logger.log(`🔍 Verificando estado de caché para: ${type}${semester ? ` (${semester})` : ''}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Descarga y cachea datos para varios semestres en una operación optimizada'
  })
  @ApiResponse({ status: 200, description: 'Precalentamiento de caché completado' })
  async warmUpCache(
    @Body() body: { 
      semesters: string[];
    ...
    try {
      ...
      for (const semester of body.semesters) {
        ...
        try {
          ...
          this.logger.error(`❌ Error precargando semestre ${semester}:`, error);
          results.push({
            semester,
            success: false,
            error: error.message
