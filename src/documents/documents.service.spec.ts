...
import { DocumentEntity, DocumentDocument, DocumentCategory } from './schemas/document.schema';
...
describe('DocumentsService', () => {
  ...
  beforeEach(async () => {
    ...
    model = module.get<Model<DocumentDocument>>(getModelToken(DocumentEntity.name));
    jest.clearAllMocks();
  ...
  describe('uploadForStudentByStaff', () => {
    it('should upload a document and save metadata', async () => {
      mockDocumentModel.new.mockImplementationOnce(dto => ({ ...dto, save: jest.fn().mockResolvedValueOnce({ ...dto, _id: documentId }) }));
      const result = await service.uploadForStudentByStaff(mockFile, createDocDto, staffId);
      // Se verifica el mock directamente, ya que 'model' es una instancia de Model<T> tipada por Mongoose.
      expect(mockDocumentModel.new).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should throw BadRequestException if no file is provided', async () => {
      ...
      await expect(service.uploadForStudentByStaff(undefined as any, createDocDto, staffId))
        .rejects.toThrow(BadRequestException);
    });
    
    it('should throw InternalServerErrorException on DB save error', async () => {
      mockDocumentModel.new.mockImplementationOnce(dto => ({ ...dto, save: jest.fn().mockRejectedValueOnce(new Error('DB error')) }));
      await expect(service.uploadForStudentByStaff(mockFile, createDocDto, staffId))
  ...
  describe('getDocumentsByStudentId', () => {
    it('should return documents for a student', async () => {
      ...
      expect(model.find).toHaveBeenCalledWith({ studentId: new Types.ObjectId(studentId) });
      expect(result).toEqual([mockDocument]);
    });
    it('should throw BadRequestException for invalid studentId', async () => {
      await expect(service.getDocumentsByStudentId('invalid-id')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getDocumentById', () => {
    ...
    it('should throw NotFoundException if document not found', async () => {
      ...
      await expect(service.getDocumentById(documentId)).rejects.toThrow(NotFoundException);
    });
    it('should throw BadRequestException for invalid documentId', async () => {
      await expect(service.getDocumentById('invalid-id')).rejects.toThrow(BadRequestException);
    });
  });
  
  describe('getDocumentFileDetails', () => {
    it('should return document details (reusing getDocumentById)', async () => {
      jest.spyOn(service, 'getDocumentById').mockResolvedValueOnce(mockDocument as DocumentDocument);
  ...
  describe('updateDocumentMetadata', () => {
    const updateDto: UpdateDocumentMetadataDto = { category: DocumentCategory.OTRO };
    it('should update document metadata', async () => {
      const updatedDoc = { ...mockDocument, ...updateDto };
      mockDocumentModel.exec.mockResolvedValueOnce(updatedDoc);
      const result = await service.updateDocumentMetadata(documentId, updateDto);
      expect(model.findByIdAndUpdate).toHaveBeenCalledWith(documentId, { $set: updateDto }, { new: true });
      expect(result).toEqual(updatedDoc);
    });

    it('should throw NotFoundException if document to update not found', async () => {
      mockDocumentModel.exec.mockResolvedValueOnce(null);
      await expect(service.updateDocumentMetadata(documentId, updateDto)).rejects.toThrow(NotFoundException);
    });
    it('should throw BadRequestException for invalid documentId', async () => {
      await expect(service.updateDocumentMetadata('invalid-id', updateDto)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('deleteDocument', () => {
    ...
    it('should throw NotFoundException if document not found by getDocumentById', async () => {
      ...
      (service.getDocumentById as jest.Mock).mockRejectedValueOnce(new NotFoundException());
      await expect(service.deleteDocument(documentId)).rejects.toThrow(NotFoundException);
    ...
    it('should log error if physical file deletion fails but still attempt DB deletion', async () => {
      ...
      (fs.unlink as jest.Mock).mockRejectedValueOnce(new Error('File system error'));
    ...
    it('should throw BadRequestException for invalid documentId', async () => {
      await expect(service.deleteDocument('invalid-id')).rejects.toThrow(BadRequestException);
    });
  });
});
