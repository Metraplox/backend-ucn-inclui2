...
export class ConsentService {
  ...
  async giveOrUpdateConsent(
    ...
    if (!Types.ObjectId.isValid(authenticatedStudentId) || !Types.ObjectId.isValid(documentId)) {
    ...
    const student = await this.studentModel.findById(authenticatedStudentId).exec();
    if (!student) {
      throw new NotFoundException(`Estudiante con ID "${authenticatedStudentId}" no encontrado.`);
    }
    ...
    if (document.studentId.toString() !== authenticatedStudentId) {
      throw new ForbiddenException('No tienes permiso para dar consentimiento sobre este documento.');
    }
  ...
  async getConsentsByStudent(authenticatedStudentId: string): Promise<ConsentDocument[]> {
    if (!Types.ObjectId.isValid(authenticatedStudentId)) {
      throw new BadRequestException('ID de estudiante inválido.');
    }
    return this.consentModel.find({ studentId: new Types.ObjectId(authenticatedStudentId) }).exec();
  }
}
