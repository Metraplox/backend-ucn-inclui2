...
const mockStudentInstance: Partial<StudentDocument> = {
  _id: studentId as any,
  nombres: 'Test Student',
};
const mockDocumentInstance: Partial<DocumentDocument> = {
...
describe('ConsentService', () => {
  ...
  beforeEach(async () => {
    ...
    consentModel = module.get<Model<ConsentDocument>>(
      getModelToken(Consent.name),
    );
  ...
  describe('giveOrUpdateConsent', () => {
    it('should create or update consent successfully', async () => {
      ...
      const result = await service.giveOrUpdateConsent(
        studentId,
        createConsentDto,
        '127.0.0.1',
        'test-agent',
    ...
    it('should throw BadRequestException for invalid studentId', async () => {
      await expect(
        service.giveOrUpdateConsent('invalid-id', createConsentDto),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException for invalid documentId', async () => {
      const dtoWithInvalidDocId = {
        ...createConsentDto,
        documentId: 'invalid-doc-id',
      };
      await expect(
        service.giveOrUpdateConsent(studentId, dtoWithInvalidDocId),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw NotFoundException if student not found', async () => {
      ...
      await expect(
        service.giveOrUpdateConsent(studentId, createConsentDto),
  ...
  describe('getConsentForDocumentByStudent', () => {
    it('should return consent if found', async () => {
      mockConsentModel.exec.mockResolvedValueOnce(mockConsentInstance);
      const result = await service.getConsentForDocumentByStudent(
        studentId,
        documentId,
      );
    ...
    it('should throw BadRequestException for invalid studentId', async () => {
      await expect(
        service.getConsentForDocumentByStudent('invalid-id', documentId),
      ).rejects.toThrow(BadRequestException);
    });
    it('should throw BadRequestException for invalid documentId', async () => {
      await expect(
        service.getConsentForDocumentByStudent(studentId, 'invalid-id'),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getConsentsByStudent', () => {
    it('should return an array of consents for a student', async () => {
      ...
      expect(consentModel.find).toHaveBeenCalledWith({
        studentId: new Types.ObjectId(studentId),
      });
      expect(result).toEqual([mockConsentInstance]);
    });
    it('should throw BadRequestException for invalid studentId', async () => {
      await expect(service.getConsentsByStudent('invalid-id')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
