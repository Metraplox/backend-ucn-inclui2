...
export class Consent {
  @ApiProperty({ description: 'ID único del registro de consentimiento (generado por MongoDB)', example: '605c72ef9167f86c2cabc999' })
  declare _id: string;

  @ApiProperty({ description: 'ID del documento asociado', example: '60c72b2f9b1d8c001f8e4a3c', type: String })
  ...
  @ApiProperty({ description: 'ID del estudiante que otorga el consentimiento', example: '605c72ef9167f86c2cabc123', type: String })
  ...
  @ApiProperty({ description: 'Estado del consentimiento (true si otorgado)', example: true })
  @Prop({ required: true })
  isConsentGiven: boolean; // true si se otorga, false si se revoca (o manejar revocación de otra manera)

  @ApiProperty({ description: 'Fecha y hora en que se registró el consentimiento/cambio', example: '2023-05-10T10:00:00.000Z' })
  @Prop({ type: Date, default: Date.now })
  consentDate: Date; // Fecha y hora en que se registró el consentimiento/cambio

  @ApiProperty({ description: 'Dirección IP desde donde se otorgó el consentimiento (auditoría)', example: '192.168.1.100', required: false })
  @Prop({ required: false, trim: true })
  ipAddress?: string; // IP desde donde se dio el consentimiento (para auditoría)

  @ApiProperty({ description: 'User agent del navegador desde donde se otorgó el consentimiento (auditoría)', example: 'Mozilla/5.0 (...)', required: false })
  @Prop({ required: false, trim: true })
