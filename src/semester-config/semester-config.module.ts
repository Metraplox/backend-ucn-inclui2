...
import { SemesterConfig, SemesterConfigSchema } from './schemas/semester-config.schema';
import { SemesterConfigController } from './semester-config.controller';
import { SemesterConfigService } from './semester-config.service';

@Module({
  imports: [
    MongooseModule.forFeature([{ name: SemesterConfig.name, schema: SemesterConfigSchema }]),
  ],
