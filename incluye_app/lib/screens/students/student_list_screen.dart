...
class _StudentListScreenState extends State<StudentListScreen> {
  ...
  Map<String, String> _careerNamesMap = {}; // Mapa para ID de carrera -> Nombre de carrera
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      print("StudentListScreen: Mapa de carreras (_careerNamesMap): $_careerNamesMap");
      if (careers.isNotEmpty) {
        print("StudentListScreen: Detalle primera carrera en mapa - ID: ${careers.first.id}, Nombre: ${careers.first.name}");
      }
      ...
      if (studentsData.isNotEmpty) {
        for (int i = 0; i < studentsData.length; i++) {
          ...
          print("StudentListScreen: Estudiante[$i]: Nombre=${student.nombreCompleto}, CarreraID=${student.rawCarreraId}, CarreraObjetoCompleto=${student.carreraIdObject}");
      ...
      print("StudentListScreen: Error cargando datos iniciales: $e\nStacktrace: $s");
      if (mounted) {
        setState(() { _isLoading = false; }); // Asegurar que el loading termine
  ...
  void _filterStudents(String query) {
    ...
    if (query.isEmpty) {
      ...
      tempList = _students.where((student) {
  ...
  Future<void> _deleteStudent(String studentDocId) async {
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Estudiante eliminado'), backgroundColor: Colors.green));
      _fetchStudentsOnly(); // Solo refrescar estudiantes
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error al eliminar estudiante'), backgroundColor: Colors.red));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Column(
              children: [
                    ...
                    children: [
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          contentPadding: const EdgeInsets.symmetric(vertical: 14.0, horizontal: 16.0), // Ajuste de padding
                          filled: true,
                          fillColor: Theme.of(context).colorScheme.surfaceVariant.withOpacity(0.5), // Color sutil
                          suffixIcon: _searchController.text.isNotEmpty
                            ? IconButton(
                                icon: const Icon(Icons.clear, size: 20),
                                onPressed: () {
                                  _searchController.clear(); // _filterStudents se llama por el listener
                                },
                              )
                            : null,
                        ),
                ...
                Expanded(
                  child: _filteredStudents.isEmpty && !_isLoading
                  ? Center(
                      child: Padding(
                  ...
                  : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String careerNameDisplay = _careerNamesMap[student.rawCarreraId] ?? 'Carrera no asignada';
                      print( "StudentListScreen: Mostrando estudiante: ${student.nombreCompleto}, Carrera: $careerNameDisplay");
                      ...
                      return ListTile(
                        leading: CircleAvatar(
                          ...
                          child: Text(
                            student.nombres.isNotEmpty ? student.nombres[0].toUpperCase() : '?',
                            style: TextStyle(color: Theme.of(context).primaryColorDark),
                          ),
                        ),
                        title: Text(student.nombreCompleto, style: const TextStyle(fontWeight: FontWeight.w500)),
                        subtitle: Text('${student.email}\n$careerNameDisplay - RUT: ${student.rut}'),
                        isThreeLine: true,
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(icon: const Icon(Icons.person_outline), tooltip: 'Ver perfil', onPressed: () {
                              Navigator.push(context, MaterialPageRoute(builder: (_) => StudentProfileScreen(studentId: student.id))).then((_) { _fetchStudentsOnly(); });
                            }),
                            IconButton(icon: const Icon(Icons.folder_open_outlined), tooltip: 'Documentos', onPressed: () {
                              Navigator.push(context, MaterialPageRoute(builder: (_) => StudentDocumentsScreen(studentId: student.id)));
                            }),
                            IconButton(icon: const Icon(Icons.edit_outlined), tooltip: 'Editar', onPressed: () async {
                              final result = await showDialog<bool>(context: context, builder: (_) => EditStudentDialog(student: student, onUpdated: (updatedStudent) { Navigator.of(context).pop(true); }));
                              if (result == true) { _fetchStudentsOnly(); }
                            }),
                            IconButton(icon: Icon(Icons.delete_outline, color: Colors.red.shade700), tooltip: 'Eliminar', onPressed: () => _deleteStudent(student.id)),
                          ],
                        ),
                        onTap: () {
                           Navigator.push(context, MaterialPageRoute(builder: (_) => StudentProfileScreen(studentId: student.id))).then((_) { _fetchStudentsOnly(); });
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.push<bool>(context, MaterialPageRoute(builder: (context) => const StudentCreateScreen()));
          if (result == true) {
            _fetchStudentsOnly();
          }
        },
