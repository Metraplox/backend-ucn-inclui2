...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    try {
      ...
      final user = await AuthService.loginWithGoogle(); // Esta es la llamada importante
      ...
      if (user != null) {
        ...
        setState(() {
          errorMessage = 'Tu cuenta de Google no está registrada en el sistema o no ha sido vinculada. Contacta al administrador.';
        });
      }
    } catch (e, s) {
      ...
      if (!mounted) return; // Verificar si el widget sigue montado
      setState(() {
        errorMessage = 'Error inesperado al intentar iniciar sesión con Google: ${e.toString()}';
      });
    } finally {
      if (mounted) {
        setState(() {
        ...
        print("LOGIN_SCREEN (_handleGoogleSignIn): _isGoogleLoading puesto a false.");
      }
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      onSubmitted: (_isLoading || _isGoogleLoading) ? null : (_) => _handleLogin(),
                      enabled: !_isLoading && !_isGoogleLoading,
                    ),
                    const SizedBox(height: 24),
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: _isLoading
                            ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation(Colors.white), strokeWidth: 3,))
                            : const Text('Ingresar'),
                      ),
                    ),
                    const SizedBox(height: 16),
