...
class _StudentCreateScreenState extends State<StudentCreateScreen> {
  ...
  Future<void> _loadCareers() async {
    ...
    try {
      ...
      log.e("StudentCreateScreen – error cargando carreras: $e");
      if (mounted) {
        setState(() {
          _isLoadingCareers = false;
        });
  ...
  Future<void> _submitForm() async {
    ...
    if (_formData['carreraId'] == null ||
        (_formData['carreraId'] as String).isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    log.d('StudentCreateScreen – payload create student: $dataToSend');
    ...
    if (success) {
      ...
      Navigator.pop(
        context,
        true,
      ); // Devuelve true para indicar éxito a la pantalla anterior
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Error al crear estudiante'),
          backgroundColor: Colors.red,
        ),
      );
    }
                    ...
                    children: [
                      ...
                      if (_availableCareers.isNotEmpty)
                        DropdownButtonFormField<String>(
                          ...
                          value:
                              _selectedCareerId, // Usa la variable de estado para el ID
                          ...
                          items:
                              _availableCareers.map((Career career) {
                                return DropdownMenuItem<String>(
                                  value:
                                      career
                                          .id, // El valor es el ID de la carrera
                                  child: Text(
                                    career.name,
                      ...
                      CheckboxListTile(
                        // Para consentimientoFirmado
                        title: const Text("Consentimiento Firmado"),
                        value:
                            _formData['consentimientoFirmado'] as bool? ??
                            false,
                        onChanged: (bool? newValue) {
                          setState(() {
                            _formData['consentimientoFirmado'] =
                                newValue ?? false;
                          });
                        },
  ...
  Widget _buildTextField(
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        validator:
            validator ??
            _requiredValidator, // Usa validador específico o el genérico
            ...
            onPressed: () async {
              ...
              if (pickedDate != null) {
                setState(() {
                  ...
                  _formData['fechaNacimiento'] =
                      formatted; // Guardar en formData
                });
              }
            },
          ),
