...
## 🚀 **VERSION 1.1.0** - *Julio 2025*

### **🏆 MEJORAS DE ROBUSTEZ Y FUNCIONALIDADES CLAVE**

#### **✅ CARACTERÍSTICAS PRINCIPALES IMPLEMENTADAS**

##### **🔑 Gestión de Contraseñas y Usuarios Mejorada**
- **Cambio de Contraseña de Usuario:** Implementado un flujo seguro para que los usuarios cambien su propia contraseña, validando la antigua.
- **Cambio de Contraseña por Administrador:** El rol `COORDINADOR` ahora puede restablecer la contraseña de cualquier usuario.
- **Gestión de Usuarios Activa:** Se habilitó la funcionalidad completa de creación, edición y eliminación de usuarios para roles administrativos.

##### **📄 Descarga de Documento de Consentimiento**
- Se implementó un endpoint seguro y dedicado (`GET /documents/templates/consent-form`) para que los estudiantes puedan descargar el formulario de consentimiento oficial.
- La interfaz de frontend se actualizó para consumir este endpoint, facilitando el acceso al documento.

##### **🔔 Notificaciones en Tiempo Real con WebSockets**
- **Refactorización del Servicio de Notificaciones:** El sistema de notificaciones del frontend fue completamente migrado de un sistema basado en `SharedPreferences` a una solución robusta en tiempo real usando **WebSockets** (`socket.io`).
- **Notificación de Seguridad:** Al cambiar la contraseña, el usuario recibe una notificación instantánea de alerta en la aplicación.
- **UI Reactiva:** La `AppBar` ahora muestra un contador de notificaciones no leídas que se actualiza en tiempo real.

##### **🛡️ Refactorización de Validaciones en Formularios**
- **Centralización de la Lógica:** Se creó una clase de utilidad `Validators` en el frontend para unificar todas las reglas de validación (email, contraseña, campos no vacíos).
- **Mejora de Formularios Críticos:** Se refactorizaron 5 formularios clave (Login, Registro de Docente, Cambio de Contraseña, Creación/Edición de Usuario) para usar los nuevos validadores y proporcionar retroalimentación instantánea al usuario (`AutovalidateMode.onUserInteraction`).
- **Aumento de la Seguridad y UX:** Se añadió la confirmación de contraseña en todos los flujos relevantes, mejorando la seguridad y la experiencia del usuario.

##### **🏗️ Refactorización Arquitectónica del Frontend**
- **Implementación del Patrón Repositorio:** Se desacopló la lógica de acceso a datos de los servicios. Se creó `AuthRepository` como primera implementación, aislando las llamadas a la API de autenticación.
- **Adopción del Patrón Singleton para Servicios:** El `AuthService` fue refactorizado a un Singleton para asegurar una única instancia a lo largo de la app, facilitando el manejo de estado y la futura inyección de dependencias.
- **Centralización de Constantes:** Se creó una estructura en `lib/core/constants` para centralizar valores clave, empezando por la URL de la API.
- **Resolución de Conflictos de Modelos:** Se unificó el modelo `User` eliminando duplicados, estableciendo `user_model.dart` como la única fuente de verdad.
...
### **v1.1.0** - *Julio 2025* - **MEJORAS DE FUNCIONALIDAD Y SEGURIDAD**

#### **✨ NUEVAS FUNCIONALIDADES**

- **Registro de Docentes (Autoservicio):**
  - **Endpoint Público:** Se ha añadido `POST /auth/register-teacher` para permitir el registro de nuevos docentes.
  - **Validación de Email:** El sistema valida que el correo electrónico pertenezca al dominio `@ucn.cl` y no sea de un alumno (`@alumnos.ucn.cl`).
  - **Asignación de Rol Automática:** Los usuarios registrados a través de este endpoint reciben automáticamente el rol `DOCENTE`.
  - **Implementación Frontend:** Se ha creado una nueva pantalla en la aplicación Flutter (`teacher_register_screen.dart`) con un formulario y la lógica necesaria para consumir el nuevo endpoint.

- **Integración de Subida de Documentos:**
  - **Lógica Frontend-Backend:** Se ha conectado la pantalla existente de subida de documentos en Flutter (`upload_document_screen.dart`) con el servicio del backend.
  - **Manejo de Archivos Multiplataforma:** Se ha mejorado el `DocumentService` en Flutter para manejar `PlatformFile`, asegurando que la subida de archivos funcione tanto en web como en móvil.
  - **Detección de Rol:** Se ha implementado una lógica segura para determinar si el usuario es un estudiante, obteniendo el rol desde los datos de sesión para usar el endpoint de subida correcto.

#### **🔒 MEJORAS DE SEGURIDAD**

- **Generación de Contraseña para Estudiantes:**
  - **Lógica de Creación:** Al crear un nuevo estudiante, la contraseña se genera automáticamente a partir de su fecha de nacimiento (formato `ddmmyyyy`).
  - **Seguridad:** La contraseña generada es inmediatamente hasheada usando `bcrypt` antes de ser almacenada en la base de datos, eliminando la necesidad de contraseñas nulas o temporales.
  - **Validación:** El campo de fecha de nacimiento ahora es obligatorio para la creación de estudiantes, asegurando la robustez del proceso.

- **Persistencia de Roles en Frontend:**
  - **Almacenamiento Seguro:** Tras el inicio de sesión, los roles del usuario ahora se almacenan en `SharedPreferences`.
  - **Toma de Decisiones en UI:** Esto permite a la aplicación mostrar u ocultar componentes de la interfaz de manera eficiente y segura. La validación final y autoritativa siempre reside en el backend a través de la verificación del token JWT.